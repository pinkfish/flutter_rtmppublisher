...
class VideoEncoder(val getVideoData: GetVideoData, val width: Int, val height: Int, var fps: Int, var bitrate: Int, val rotation: Int, val hardwareRotation: Boolean, val iFrameInterval: Int, val formatVideoEncoder: FormatVideoEncoder, val avcProfile: Int = -1, val avcProfileLevel: Int = -1) : BaseEncoder(), GetCameraData {
    ...
    override protected fun stopImp() {
     ...
     */
    override  protected fun chooseEncoder(mime: String): MediaCodecInfo? {
...
override    protected fun getInputFrame(): Frame
         {
            val frame: Frame = queue.take()
            if (fpsLimiter.limitFPS()) return inputFrame
            ...
            if (!hardwareRotation) {
                ...
                buffer = if (isYV12) YUVUtil.rotateYV12(buffer, width, height, orientation) else YUVUtil.rotateNV21(buffer, width, height, orientation)
            }
            buffer = if (isYV12) YUVUtil.YV12toYUV420byColor(buffer, width, height, formatVideoEncoder) else YUVUtil.NV21toYUV420byColor(buffer, width, height, formatVideoEncoder)
    ...
    override    protected fun checkBuffer( byteBuffer: ByteBuffer,
    ...
    override    protected fun sendBuffer( byteBuffer: ByteBuffer,
                              bufferInfo: MediaCodec.BufferInfo) {
